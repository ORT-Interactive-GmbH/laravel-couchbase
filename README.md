...
```php
$app->register(ORT\Interactive\Couchbase\CouchbaseServiceProvider::class);
...
{
    return new ORT\Interactive\Couchbase\Connection($config);
});
```
...
```php
'CouchbaseModel'       => 'ORT\Interactive\Couchbase\Eloquent\Model',
```
...
If you're using a hybrid Couchbase and SQL setup, you're in luck! The model will automatically return a Couchbase- or SQL-relation based on the type of the related model. Of course, if you want this functionality to work both ways, your SQL-models will need use the `ORT\Interactive\Couchbase\Eloquent\HybridRelations` trait. Note that this functionality only works for hasOne, hasMany and belongsTo relations.
